define('components/gf-order-unget/gf-order-unget', function(require, exports, module) {
  ...
  var order = avalon.define({
      ...
      '$data-box_config': {
          store: 'order-unget',
          actionBtns: {
              modifyOrder_no: '<a href="javascript:;" class="btn btn-info btn-xs" ms-click="actions.modifyOrder_no(el)"><i class="fa fa-edit"></i>修改运单号</a>',
          ...
          actions: {
              ...
              modifyOrder_no: function (record) {
                  dialogVm.title = '修改运单信息';
                  dialogVm.record = { id: record.id, logistics_no: record.logistics_no };
                  dialogVm.$post = function (package) {
                      ...
                      var orderStore = store[order['$data-box_config'].store];
                      store['order-unget'].modifyOrder_no({ id: package.record.id, logistics_no: package.record.logistics_no }).then(function (r) {
                            if (r.code == '0') {
                              avalon.vmodels['data-box_order_unget'].loadData();
