function defineNodeSql(executor){
	if(executor.isOracle()){
		...
		D2JS.prototype.nodeSql = function (params, types, cutCond, orderBy, reverse){
			var sql = 'with nd as (select id, name, parent_id, type, level lv from node connect by parent_id = prior id ';
			if(reverse){
				sql = 'with ndr as (select id, name, parent_id, type, level lv from node connect by prior parent_id = id ';
			}
		...
		D2JS.prototype.nodeSql = function (params, types, cutCond, orderBy, reverse){
			...
			var alias = 'nd';
			var sql = 'with recursive t as (select *, 1 lv, name::varchar(2000) path from node where id = :' + nodeProp 
...
function initTypeRelations(){
	...
	if(allowedChildTypes == null){
		...
		var typeRelations = d2js.executor.query('select * from NODE_ALLOWED_CHILDREN_TYPE',[], true).rows;
