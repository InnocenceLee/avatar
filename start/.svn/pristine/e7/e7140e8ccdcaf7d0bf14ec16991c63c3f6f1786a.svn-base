...
<script
	src="${pageContext.request.contextPath}/js/common/readTreeJson.js"
...
<%!String createSysFunLink(Map fun, int level, HttpServletRequest request) {
		...
		if ((fun.get("open_mode")).equals("W") || (fun.get("open_mode")).equals("w")) {
			...
			return "<a href='###' fun-uri='" + request.getContextPath() + fun.get("uri") + "' fun-code='"
					+ fun.get("code") + "'>" + fun_name + "</a>";
		} else if ((fun.get("open_mode")).equals("N")) {
			return "<a href='###' fun-code='" + fun.get("code") + "'>" + fun_name + "</a>";
		} else if ((fun.get("open_mode")).equals("C")) {
			return "<a href='" + request.getContextPath() + "/front/main.jssp?fun=" + fun.get("code")
					+ "' data-molecule='true' fun-uri='" + fun.get("uri") + "' fun-code='" + fun.get("code") + "'>"
					+ fun_name + "</a>";
		} else if ((fun.get("open_mode").equals("P"))) {
			return "<a href='" + request.getContextPath() + "" + fun.get("uri") + "' fun-code='" + fun.get("code")
					+ "'>" + fun_name + "</a>";// Modal 
		} else {
			return "";
		}
...
<div class="page-sidebar nav-collapse collapse">
	...
	<ul class="page-sidebar-menu">
		...
		%>
		<li class=""><a href="###" fun-code="<%=curr.get("code")%>">
				<%=getIcon(curr)%><span class="title"><%=curr.get("name")%></span>
			...
			<ul class="sub-menu">
				...
				%>
				<li class="myli1"><%=createSysFunLink(currFunc, 2, request)%>
				</li>
