var UIGeneral = function () {
  ...
  //modify by lyh 增加了参数，传递总页数和当前页数
    var handleDynamicPagination = function(page,total,maxVisible) {
/*        $('#dynamic_pager_demo1').bootpag({
        ...
        $('#dynamic_pager').bootpag({
        	...
        	//点击翻页事件
        	//console.log(event);
        	tableAjax(num,maxVisible,false);
        });
    }
    //end
    return {
        //main function to initiate the module
        init: function (page,total,maxVisible,modelName) {
            handlePulsate();
            handleGritterNotifications();
            //modify by lyh 增加了参数，传递总页数,当前页数,最大页面数量，模块代码
            handleDynamicPagination(page,total,maxVisible,modelName);
            //end
        }
